...
public class Bot {
    ...
    private int obstacleAmount(List<Object> blocks) {
        int amount = 0;
        for (Object o : blocks) {
            if (o == Terrain.MUD || o == Terrain.OIL_SPILL) {
                amount ++;
            } else if (o == Terrain.WALL) {
                amount += 2;
            }
        }
        return amount;
    ...
    public Command run(GameState gameState) {
        /* TODO: Logika untuk mengecek apakah harus OIL_SPILL? */
    ...
    private ArrayList<Integer> getLaneScore(GameState gameState, String type) {
        ...
        for (int i = blocksToSkip + 1; j < checkDistance && i < laneList.length; i ++) {
            ...
            if (laneList[i].terrain == Terrain.MUD || laneList[i].terrain == Terrain.OIL_SPILL) {
                ...
                finalSpeed = 0;
                laneDamage += 12;
            } else if (laneList[i].occupiedByPlayerId == opponent.id && getAcceleratingSpeed(opponent.speed, 2) < checkDistance) {
                /* Jika memungkinkan, jangan ke lane yang dihalangi oleh opponent */
                laneDamage += 1;
            } else if (laneList[i].terrain == Terrain.OIL_POWER) {
                pickUpScore += 1;
            } else if (laneList[i].terrain == Terrain.TWEET || laneList[i].terrain == Terrain.EMP || laneList[i].terrain == Terrain.LIZARD) {
                pickUpScore += 2;
